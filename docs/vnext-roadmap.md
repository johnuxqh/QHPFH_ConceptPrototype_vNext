...
P4.06 — Unified Status + Severity Styling
- Status: Complete
- Outcome: Unified Status + Severity Styling created to standardise critical, high, warning, medium, info, low, success, ready, pending, blocked, closed/unavailable, neutral, and unknown operational state presentation across shared badges, chips, cards, tables, banners, KPI indicators, and Bed/Ward indicators.
- Proof-of-concept migration: Operational awareness banner severity labels, shared KPI status chips, shared insight priority badges, Bed Management status tags, and Ward Operations bed-state row markers now consume central status/severity tokens while preserving existing labels, icons, status names, filtering, sorting, and workflow actions.
- Deferred migration: Remaining page-local badges, slideout statuses, Allocation Centre statuses, Delayed Discharge workflow states, utility drawer indicators, and legacy colour-only markers should migrate progressively in later prompts without changing operational meaning or data hierarchy.
